...
data Stmt' a
    ...
    | ArrDecl a (Type' a) (ArrExpr' a)
    | Ass a Ident (Expr' a)
    | ArrAss a (ArrExpr' a) (Expr' a)
    | Incr a Ident
    | Decr a Ident
    | Ret a (Expr' a)
    | VRet a
...
type ArrExpr = ArrExpr' BNFC'Position
data ArrExpr' a
    = FirstDim a Ident (Expr' a) | MultDim a (ArrExpr' a) (Expr' a)
  deriving (C.Eq, C.Ord, C.Show, C.Read, C.Functor, C.Foldable, C.Traversable)
...
data Lambda' a = LambdaDef a (Type' a) [Arg' a] (Block' a)
  deriving (C.Eq, C.Ord, C.Show, C.Read, C.Functor, C.Foldable, C.Traversable)
...
instance HasPosition ArrExpr where
  hasPosition = \case
    FirstDim p _ _ -> p
    MultDim p _ _ -> p
